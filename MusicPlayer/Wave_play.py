    ...
    def play_lrc(self,filename):
        ...
        while self.__running.isSet():
            ...
            if lrc == None:
                ...
                file.write(lrc)
                file_full = open("full.txt",'r')
                i = 0
                for line in file_full.readlines():
                    #print(line)
                    if str(lrc + '\n') == line:
                        self.cur_lrc = lrc
                        self.cur_line = i
...
class Wave_Player:
    def __init__(self, filename, lrcname):

        self.filename = 'data/' + filename
        self.lrc_file = 'data/' + lrcname
        # print(self.lrc_file)
        if (not os.path.exists(self.lrc_file)) or lrcname == '':
            self.lrc_file = 'data/default.lrc' 
